# This file contains the items that will be inserted into the database_setup.py file.
...
#Create a session in order to insert data into the Store Items Inventory database.
DBSession = sessionmaker(bind=engine)
session = DBSession()

#Create a test user.
User1 = User(name="Ebony N. Plummer", email="ebony.plummer@keepitsts.com", picture="#")
...
storeItem1 = StoreItem(user_id=1, name="Python Crash Course", author="Eric Matthes", genre="Computer Programming",
...
storeItem2 = StoreItem(user_id=1, name="Automate the Boring Stuff with Python", author='Al Sweigart',
             genre="Compute Programming", price="$25.00", 
             description="A practical guide to programming for total beginners.", category=category1)

session.add(storeItem2)
session.commit()

storeItem3 = StoreItem(user_id=1, name="Don Quixote", author="Miguel de Cervantes", genre="Classic Literature",\
             price="$15.00", description="""Alonso Quixano, a retired country gentleman in his fifties, 
...
storeItem4 = StoreItem(user_id=1, name="A Brief History of Time", author="Stephen Hawking", genre="Science", price="$15.00",\
             description="""This essential book on cosmology explains complex concepts such as space, time, and
             ...
             the complexity of the universe.""", category=category1)

session.add(storeItem4)
session.commit()

storeItem5 = StoreItem(user_id=1, name="Catch-22", author="Joseph Heller", genre="Classic Fiction", price="$10.00",\
             description="""Catch-22 is a satirical, historical novel by the American author Joseph Heller, first
             ...
             of sequence so that the time line develops along with the plot.""", category=category1)
...
storeItem6 = StoreItem(user_id=1, name="Sgt. Peppers Lonely Hearts Club Band", author="The Beatles", genre="Rock",\
             price="10.00",description="Year Released: 1967", category=category2)

session.add(storeItem6)
session.commit()

storeItem7 = StoreItem(user_id=1, name="1999", author="Prince", genre="R&B/Soul", price="$10.00",\
             description="Year Released: 1982",category=category2)

session.add(storeItem7)
session.commit()


storeItem8 = StoreItem(user_id=1, name="21", author="Adele", genre="Pop", price="$10.00", description="Year Release: 2011",\
             category=category2)

session.add(storeItem8)
session.commit()

storeItem9 = StoreItem(user_id=1, name="Songs in the Key of Life", author="Stevie Wonder", genre="R&B/Soul", price="10.00",\
             description="Year Released: 1976", category=category2)

session.add(storeItem9)
session.commit()

storeItem10 = StoreItem(user_id=1, name="The Planets", author="Gustave Holst", genre="classical", price="$10.00",\
              description="Year Premiere: 1918", category=category2)
...
storeItem11 = StoreItem(user_id=1, name="Citizen Kane (1941)", author="Orson Wells", genre="Drama", price="$10.00",\
              description="""Stars: Orson Welles; Joseph Cotten; Everett Sloane, Agnes Moorehead
              ...
              character.""", category=category3)

session.add(storeItem11)
session.commit()

storeItem12 = StoreItem(user_id=1, name="All about Eve (1950", author="Joseph L. Mankiewicz", genre="Drama", price="$10.00",\
              ...
              his scene-stealing consort.""", category=category3)

session.add(storeItem12)
session.commit()

storeItem13 = StoreItem(user_id=1, name="Dr. Strangelove, Or: How I Learned To Stop Worrying And Love The Bomb (1964)",\
              author="Stanley Kubrick", genre="Comedy", price="$10.00", description="""Stars: Peter Sellers; 
              ...
              a missile. "Gentlemen, you can't fight in here! This is the War Room!""", category=category3)

session.add(storeItem13)
session.commit()

storeItem14 = StoreItem(user_id=1, name="Fantasia (1940)", author="James Algar, Ben Sharpsteen, Walt Disney and others",\
              genre="Animation", price="$7.50", description="""Stars: Deems Taylor (narrator); Leopold Stokowski
              ...
              "Rite of Spring.""", category=category3)

session.add(storeItem14)
session.commit()

storeItem15 = StoreItem(user_id=1, name="Star Wars (1977)", author="George Lucas", genre="Science Fiction", price="$10.00",\
              description="""Stars: Mark Hamill; Harrison Ford; Carrie Fisher; Alec Guinness 
              ...
              galaxy from the Dark Side. "May the force be with you." Two sequels and prequels followed.""",\
              category=category3)
