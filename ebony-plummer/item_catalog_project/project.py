...
def gconnect():
    ...
    if stored_access_token is not None and googleUser_id == stored_googleUser_id:
        response = make_response(json.dumps('Current user is already connectes'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def gdisconnect():
    ...
    if access_token is None:
        ...
        response = make_response(json.dumps('Curent user is not connected'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def showItems(category_id):
    ...
    items = session.query(StoreItem).filter_by(category_id=category_id).all()
    if 'username' not in login_session:
        return render_template('publicCatalogItems.html', items=items, category=category, creator=creator)
    else:
...
@app.route('/catalog/category/<int:category_id>/inventory/new/', methods=['GET','POST'])
def newStoreItem(category_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    if login_session['user_id'] != category.user_id:
        return "<script>function myfunction() {alert'You are not authorized to add items this category. Please create your own category in order to add items.';}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        newItem = StoreItem(
...
#Edit a store item.
@app.route('/catalog/category/<int:category_id>/inventory/<int:inventory_id>/edit/', methods=['GET', 'POST'])
def editStoreItem(category_id, inventory_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editItem.html', category_id=category_id, inventory_id=inventory_id,  item=editedItem)
...
def deleteStoreItem(category_id, inventory_id):
    ...
    if login_session['user_id'] != category.user_id:
        return "<script>function myfunction() {alert'You are not authorized to delete items this category. Please create your own category in order to delte items.';}</script><body onload='myFunction()''>"
    if request.method == 'POST':
